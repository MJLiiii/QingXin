/* 生成 data/featured.json —— 首页推荐池。
   口径:注释文件中 赏析非空 且 (注释或译文非空) 的诗(保证 hero 点进去内容最全)。
...
console.log(`featured.json:池 ${rows.length} 首(唐诗 ${shi} + 宋词 ${rows.length - shi}),` +
  `扫描注释 ${scanned},合格 ${eligible.size}${eligible.size === rows.length ? '' : `(${eligible.size - rows.length} 个 id 不在索引中!)`}`);
